    ...
    public function rentCar(RentCarRequest $request)
    {
        ...
        if ($car->users()->count() && $user->cars()->count()) {
            return response()->json(array(
                'data' => [
                    'status' => false ,
                    'message' => 'Невозможно арендовать больше 1-го автомобиля'
                ]
            ));
            ...
            return response()->json(array(
                'data' => [
                    'status' => false ,
                    'message' => (sprintf('Автомобиль %s %s c с номером %s занято!',$car->model->brand->brand_name,$car->model->model_name, $car->number))
                ]
            ));
            ...
            return response()->json(array(
                'data' => [
                    'status' => false ,
                    'message' => (sprintf('Пользователь %s уже арендовал другой автомобиль!',$user->name))
                ]
            ));
        }
    ...
    public function terminateRent(TerminateRentCarRequest $request)
    {
        ...
        if (!$car->users()->count()){
            return response()->json(array(
                'data' => [
                    'status' => false ,
                    'message' => (sprintf('Автомобиль %s %s с номером %s свободно',$car->model->brand->brand_name,$car->model->model_name, $car->number))
                ]
            ));
        }
